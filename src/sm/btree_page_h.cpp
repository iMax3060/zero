                                ...
                                const bool ghost)           // Should the fence key record be a ghost
{
    ...
    if (steal_src1) {
        _steal_records(steal_src1, steal_from1, steal_to1,
                       ((true == full_logging) && (true == log_src_1)) ? true
                                                                       : false);  // Turn on full logging if need to log src 1
    }
    if (steal_src2_pid0) {
    ...
    if (steal_src2) {
        _steal_records(steal_src2, steal_from2, steal_to2,
                       ((true == full_logging) && (false == log_src_1)) ? true
                                                                        : false);  // Turn on full logging if need to log src 2
    }
    ...
    for (int i = steal_from; i < steal_to; ++i) {
        ...
        if (is_leaf()) {
            ...
            if (true == full_logging) {
                ...
                // el: non-key portion only
                Logger::log_p<btree_insert_nonghost_log>(this, keystr, el, true /*is_sys_txn*/);
...
// out: data length
{
    ...
    if (rec_count > nrecs()) {
        ...
        W_FATAL_MSG(eINTERNAL,
                    << "copy_records: page rebalance operation, does not have enough records in the source page, ask for "
    ...
    for (copied_count = 0; copied_count < rec_count;) {
        ...
        if (false == keystr.copy_from_vec(key)) {
            // OOM
            W_FATAL_MSG(fcOUTOFMEMORY,
                        << "Failed to copy_records for page rebalance log record generarion purpose due to OOM");
        }
        ...
        const char* record_data)       // In: Data buffer
{
    ...
    for (count = 0; count < move_count; ++count) {
                                 ...
                                 v))                                    // truncate key + record data
        {
            ...
            W_FATAL_MSG(eINTERNAL,
                        << "btree_page_h::insert_records_dest_redo - failed to insert a record into destination page (foster child)");
        }
    }
...
inline int btree_page_h::_compare_slot_with_key(int slot, const void* key_noprefix, size_t key_len,
                                                poor_man_key key_poor) const {
...
rc_t btree_page_h::replace_ghost(const w_keystr_t& key,
    ...
    if (!redo) {
        Logger::log_p<btree_insert_log>(this, key, elem, false /*is_sys_txn*/);
    }
...
bool btree_page_h::check_chance_for_norecord_split(const w_keystr_t& key_to_insert) const {
    ...
    w_assert1(key_to_insert_len >= prefix_len && ::memcmp(get_prefix_key(), key_to_insert_raw, prefix_len)
                                                 == 0); // otherwise why to insert to this page?
...
// TODO for fair comparison with shore-mt, let's disable no-record-split for now
    ...
    for (slotid_t boundary = start_point; boundary < end_point; ++boundary) {
        if (is_leaf()) {
            ...
            size_t common_bytes = w_keystr_t::common_leading_bytes((const unsigned char*)k1, len1,
                                                                   (const unsigned char*)k2, len2);
            ...
            if (common_bytes + 1 < sep_length
                || (common_bytes + 1 == sep_length
                    && boundary == center_point)) { // if tie, give a credit to center_point
...
w_keystr_t btree_page_h::recalculate_fence_for_split(slotid_t right_begins_from) const {
    ...
    if (is_leaf()) {
        ...
        size_t common_bytes = w_keystr_t::common_leading_bytes((const unsigned char*)k1, len1, (const unsigned char*)k2,
                                                               len2);
        w_assert1(common_bytes < len2); // otherwise the two keys are the same.
...
btree_page_h::leaf_stats(btree_lf_stats_t& _stats) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        _stats.entry_overhead_bs += page()->item_space(n + 1) - (rec.key().get_length_as_keystr() - prefix_length)
                                    - rec.elen();
    }
    return RCOK;
}
...
bool btree_page_h::_is_consistent_keyorder() const {
    ...
    if (is_leaf()) {
        ...
        for (slotid_t slot = 1; slot < recs; ++slot) {
            ...
            if (w_keystr_t::compare_bin_str(prevkey, prevkey_len, curkey, curkey_len)
                >= 0) { // this time must not be equal either
                w_assert3(false);
                return false;
            }
            prevkey = curkey;
