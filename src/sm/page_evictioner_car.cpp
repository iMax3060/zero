...
{
    _clocks = new multi_clock<bf_idx, bool, 2, 0>(_bufferpool->_block_cnt);
...
bf_idx page_evictioner_car::pick_victim() {
    ...
    while (!evicted_page) {
        ...
        if ((_clocks->size_of(T_1) >= std::max<uint32_t>(uint32_t(1), _p) || blocked_t_2 >= _clocks->size_of(T_2)) && blocked_t_1 < _clocks->size_of(T_1)) {
            bool t_1_head = _clocks->get_head(T_1);
            bf_idx t_1_head_index = _clocks->get_head_index(T_1);
            ...
            bool t_2_head = _clocks->get_head(T_2);
            bf_idx t_2_head_index = _clocks->get_head_index(T_2);
            ...
            if (!t_2_head) {
                ...
                if (evicted_page) {
                    t_2_head_index = _clocks->remove_head(T_2);
