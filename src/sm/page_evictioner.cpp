...
//    u_int32_t before_p = _p;
    if (!_b1->contains(pid) && !_b2->contains(pid)) {
        ...
        DBG1(<< "Added to T_1: " << b_idx << "; New size: " << _clocks->size_of(T_1) << "; Free frames: " << _bufferpool->_approx_freelist_length);
        ...
        DBG1(<< "Added to T_2: " << b_idx << "; New size: " << _clocks->size_of(T_2) << "; Free frames: " << _bufferpool->_approx_freelist_length);
        _clocks->set(b_idx, false);
    } else {
        ...
        if ((_clocks->size_of(T_1) >= std::max(u_int32_t(1), _p) || blocked_t_2 >= _clocks->size_of(T_2)) && blocked_t_1 < _clocks->size_of(T_1)) {
            ...
            if (!t_1_head) {
                ...
                if (evicted_page) {
                    ...
                    DBG1(<< "Removed from T_1: " << t_1_head_index << "; New size: " << _clocks->size_of(T_1) << "; Free frames: " << _bufferpool->_approx_freelist_length);
                    DO_PTHREAD(pthread_mutex_unlock(&_lock));
                ...
                DBG1(<< "Removed from T_1: " << t_1_head_index << "; New size: " << _clocks->size_of(T_1) << "; Free frames: " << _bufferpool->_approx_freelist_length);
                DBG1(<< "Added to T_2: " << t_1_head_index << "; New size: " << _clocks->size_of(T_2) << "; Free frames: " << _bufferpool->_approx_freelist_length);
                continue;
            }
            ...
            if (!t_2_head) {
                ...
                if (evicted_page) {
                    ...
                    DBG1(<< "Removed from T_2: " << t_2_head_index << "; New size: " << _clocks->size_of(T_2) << "; Free frames: " << _bufferpool->_approx_freelist_length);
                    DO_PTHREAD(pthread_mutex_unlock(&_lock));
...
bf_idx page_evictioner_cart::pick_victim() {
    ...
    while (!evicted_page) {
        ...
        while (t_2_head._referenced) {
            ...
            DBG1(<< "Moved from T_2 to T_1: " << t_2_head_index << "; |T_1|: " << _clocks->size_of(T_1) << "; |T_2|: " << _clocks->size_of(T_2));
        ...
        if ((_clocks->size_of(T_1) >= std::max(u_int32_t(1), _p) || blocked_t_2 > _clocks->size_of(T_2))
         && blocked_t_1 <= _clocks->size_of(T_1)) {
            ...
            if (evicted_page) {
                ...
                DBG1(<< "Removed from T_2: " << t_1_head_index << "; |T_2|: " << _clocks->size_of(T_2) << "; Free frames: " << _bufferpool->_approx_freelist_length);
                w_assert0(_b2->insert_back(evicted_pid));
