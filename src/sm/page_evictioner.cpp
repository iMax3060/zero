...
PageEvictioner::PageEvictioner(BufferPool* bufferPool, const sm_options& options) :
...
bool PageEvictioner::evictOne(bf_idx victim) {
    bf_tree_cb_t& victimControlBlock = _bufferPool->getControlBlock(victim);
    ...
    _bufferPool->getHashtable()->erase(victimControlBlock._pid);
...
bool PageEvictioner::unswizzleAndUpdateEMLSN(bf_idx victim) {
    ...
    bf_idx_pair victimPair = *(_bufferPool->getHashtable()->lookupPair(victimPageID));
    ...
    if (parent == 0) {
        return !_bufferPool->usesWriteElision() && !victimControlBlock._swizzled;
    }
    ...
    bf_tree_cb_t& parentControlBlock = _bufferPool->getControlBlock(parent);
    ...
    /* Get the parent page. */
    w_assert1(_bufferPool->isActiveIndex(parent));
    generic_page* parentPage = _bufferPool->getPage(parent);
    ...
    //==================================================================================================================
    if (_enabledSwizzling & victimControlBlock._swizzled) {
        bool successfullyUnswizzled = _bufferPool->unswizzlePagePointer(parentPage, victimSlotID);
    ...
    generic_page* victimPage = _bufferPool->getPage(victim);
    ...
    if (_maintainEMLSN && oldVictimLSN < newVictimLSN) {
        ...
        _bufferPool->sxUpdateChildEMLSN(parentPageHandle, victimSlotID, newVictimLSN);

        w_assert1(parentPageHandle.get_emlsn_general(victimSlotID) == _bufferPool->getPage(victim)->lsn);
    }
...
void PageEvictioner::flushDirtyPage(const bf_tree_cb_t& victimControlBlock) {
     ...
     * try the fix again. */
    generic_page* victimPage = _bufferPool->getPage(_bufferPool->getIndex(victimControlBlock));
...
void PageEvictioner::do_work() {
    while (_bufferPool->getFreeList()->getCount() < _evictionBatchSize) {
