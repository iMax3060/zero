...
bool mcs_rwlock::attempt_read() {
    ...
    if (old_value & WRITER ||
        !lintel::unsafe::atomic_compare_exchange_strong(const_cast<unsigned int*>(&_holders), &old_value,
                                                        old_value + READER)) {
        return false;
    }
...
// thus defined in config/shore-config.h
    ...
    bool result = lintel::unsafe::atomic_compare_exchange_strong(const_cast<unsigned int*>(&_holders), &expected,
                                                                 WRITER);
