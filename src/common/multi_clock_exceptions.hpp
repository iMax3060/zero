...
namespace zero::multi_clock {
    ...
    /*!\class   MultiHandedClockMultiException
     * \brief   Exception thrown when multiple exceptions happen at the same time in a \link MultiHandedClock \endlink
     * \details Represents multiple exceptions that can be thrown in instances of \link MultiHandedClock \endlink. The
     *          exceptions of type \link MultiHandedClockException \endlink that would be thrown are stored in this
     *          exception.
     *
     * @tparam key_type            The key type of the corresponding \link MultiHandedClock \endlink.
     * @tparam value_type          The value type of the corresponding \link MultiHandedClock \endlink.
     ...
     */
    template<class key_type, class value_type, uint32_t clock_count, key_type invalid_index, key_type invalid_clock_index = clock_count>
    class MultiHandedClockMultiException : public MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index> {
    public:
        using MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index>::MultiHandedClockException;

        const char* what() const noexcept override {
            size_t whatStringLength = 0;
            for (MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index> exception : _exceptions) {
                whatStringLength += std::strlen(exception.what()) + 1;
            }
            char* whatSentence = new char[whatStringLength + 1];
            for (MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index> exception : _exceptions) {
                std::strcat(whatSentence, exception.what());
                std::strcat(whatSentence, "\n");
            }
            return whatSentence;
        };
         ...
         */
        typename std::vector<MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index>>::const_iterator
        getExceptions() const noexcept {
            return _exceptions.begin();
        };

        /*!\fn      addException(MultiHandedClockException exception) noexcept
         * \brief   Adds the specified exception to this \link MultiHandedClockMultiException \endlink
         * \details The specified \c exception gets appended to \link _exceptions \endlink.
         *
         * @param exception The exception to add to this \link MultiHandedClockMultiException \endlink.
         */
        void addException(const MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index> exception) noexcept {
            _exceptions.push_back(exception);
        };

        /*!\fn      addException(MultiHandedClockMultiException multiException) noexcept
         * \brief   Adds the specified exceptions to this \link MultiHandedClockMultiException \endlink
         * \details The contained exceptions of the specified \c multiException get appended to
         *          \link _exceptions \endlink.
         *
         * @param multiException The exceptions to add to this \link MultiHandedClockMultiException \endlink.
         */
        void addException(const MultiHandedClockMultiException<key_type, value_type, clock_count, invalid_index, invalid_clock_index> multiException) noexcept {
            _exceptions.insert(_exceptions.end(), multiException._exceptions.begin(), multiException._exceptions.end());
        };
    ...
    protected:
         ...
         * \details A list of exceptions that would have been thrown at the same time in one instance of
         *          \link MultiHandedClock \endlink.
         */
        std::vector<MultiHandedClockException<key_type, value_type, clock_count, invalid_index, invalid_clock_index>> _exceptions;
