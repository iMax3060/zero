...
"Mmap could not map aligned memory")
...
"Device is too large for OS file interface")
...
"Maximum number of stores already created")
...
"Bad argument or combination of arguments to function")
...
"Multiple update threads not allowed for this operation")
...
"In active transaction (not allowed for this operation)")

X(eNOABORT,

"Logging is turned off -- cannot roll back")
...
"Transaction is not participating in external 2-phase commit ")
...
"Attempt to acquire/maintain latch in Q mode failed")
...
"Need parent and child page fixed in SH or EX modes")
...
"Retry the request (used internally)")

X(eGOODRETRY,

"Retry, but had some progress (used internally)")

X(eLOCKRETRY,

"Retry related to locks (used internally)")

X(eTOOMANYRETRY,

"This IS an error and can be thrown to user")

X(eCANTWHILEACTIVEXCTS,
...
X(eBADMASTERCHKPTFORMAT,

"Bad format for master checkpoint")

X(eLOGSPACEWARN,

"Log space warning (not necessarily out)")

X(eBADCOMPENSATION,

"Log could not apply requested compensation")
...
"Causes a loop in write order dependency")

X(eLIL_TOOMANYVOL_XCT,

"Accessing too many volumes in one transaction")

X(eLIL_TOOMANYST_XCT,

"Accessing too many stores in one transaction")

X(eBF_DIRECTFIX_SWIZZLED_PTR,

"Requested a direct page fix with swizzled pointer")

X(eWRONG_PAGE_LSNCHAIN,

"Not the right per-page LSN chain to follow")
...
"Buffer pool is full during Recovery operation")
...
"Retrieved page from backup file was incorrect")
...
X(se_VOLUME_NOT_FOUND,

"se_VOLUME_NOT_FOUND")

X(se_INDEX_NOT_FOUND,

"se_INDEX_NOT_FOUND")

X(se_TABLE_NOT_FOUND,

"se_TABLE_NOT_FOUND")

X(se_TUPLE_NOT_FOUND,

"se_TUPLE_NOT_FOUND")

X(se_NO_CURRENT_TUPLE,

"se_NO_CURRENT_TUPLE")

X(se_CANNOT_INSERT_TUPLE,

"se_CANNOT_INSERT_TUPLE")

X(se_SCAN_OPEN_ERROR,

"se_SCAN_OPEN_ERROR")

X(se_INCONSISTENT_INDEX,

"se_INCONSISTENT_INDEX")

X(se_OPEN_SCAN_ERROR,

"se_OPEN_SCAN_ERROR")

X(se_LOAD_NOT_EXCLUSIVE,

"se_LOAD_NOT_EXCLUSIVE")

X(se_ERROR_IN_LOAD,

"se_ERROR_IN_LOAD")

X(se_ERROR_IN_IDX_LOAD,

"se_ERROR_IN_IDX_LOAD")
