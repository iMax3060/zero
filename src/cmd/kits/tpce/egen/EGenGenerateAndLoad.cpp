...
CGenerateAndLoad::CGenerateAndLoad(CInputFiles inputFiles,
        ...
        : m_inputFiles(inputFiles), m_iStartFromCustomer(iStartFromCustomer), m_iCustomerCount(iCustomerCount),
          ...
          m_pOutput(pOutput), m_pLogger(pLogger),
          m_LoaderSettings(iTotalCustomers, iTotalCustomers, iStartFromCustomer, iCustomerCount, iScaleFactor,
                           iDaysOfInitialTrades), m_bCacheEnabled(bCacheEnabled) {
...
void CGenerateAndLoad::GenerateAndLoadLastTrade() {
    bool bRet;
    CLastTradeTable Table(m_inputFiles, m_iCustomerCount, m_iStartFromCustomer, m_iHoursOfInitialTrades);
...
void CGenerateAndLoad::InitAddress() {
    ...
    addressTable = new CAddressTable(m_inputFiles, m_iCustomerCount, m_iStartFromCustomer,
                                     m_iStartFromCustomer != iDefaultStartFromCustomer);
    // do not generate exchange and company addresses
    // if the starting customer is not 1
}
...
void CGenerateAndLoad::InitCustomerAccountAndAccountPermission() {
    customerAccountsAndPermissionsTable = new CCustomerAccountsAndPermissionsTable(m_inputFiles, m_iLoadUnitSize,
                                                                                   m_iCustomerCount,
                                                                                   m_iStartFromCustomer);
}
...
void CGenerateAndLoad::InitNewsItemAndNewsXRef() {
    newsItemAndXRefTable = new CNewsItemAndXRefTable(m_inputFiles, m_iCustomerCount, m_iStartFromCustomer,
                                                     m_iHoursOfInitialTrades);
}
