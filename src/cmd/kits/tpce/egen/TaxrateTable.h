        ...
        */
        bool GenerateNextRecord() {
            ...
            if (m_iCountryIndex < m_pCountry->GetSize()) {
                ...
                if (m_iCountrySubIndex >= m_pCountry->GetRecord(
                        m_iCountryIndex)->size()) {   // All elements of the current record have been traversed
                    // Advance to the next record in the file
                    ++m_iCountryIndex;
                    m_iCountrySubIndex = 0; // start with the first element in the record's vector
                }
            } else {   // Must try to return a Division record
                if (m_iDivisionIndex < m_pDivision->GetSize()) {
                    pRatesRecord = m_pDivision->GetRecord(
                            m_iDivisionIndex);    // get into a separate variable for convenience
                    ...
                    if (m_iDivisionSubIndex >= m_pDivision->GetRecord(
                            m_iDivisionIndex)->size()) {   // All elements of the current record have been traversed
                        // Advance to the next record in the file
                        ++m_iDivisionIndex;
