...
//check the cardinality of all tables to see if it is correctly generated
    ...
    void testInputs() {
        ...
        for (int i = 0; i < 2; i++) {
            trade_lookup_input_t in41 = create_trade_lookup_input(0, 0);
            in41.print();
        }
        for (int i = 0; i < 2; i++) {
            market_watch_input_t in5 = create_market_watch_input(0, 0);
            in5.print();
        }
        for (int i = 0; i < 2; i++) {
            security_detail_input_t in6 = create_security_detail_input(0, 0);
            in6.print();
        }
        trade_status_input_t in7 = create_trade_status_input(0, 0);
        in7.print();
        for (int i = 0; i < 2; i++) {
            trade_update_input_t in8 = create_trade_update_input(0, 0);
            in8.print();
        }
        for (int i = 0; i < 2; i++) {
...
/** Construction  */
    ShoreTPCEEnv::ShoreTPCEEnv(boost::program_options::variables_map map) :
            ShoreEnv(map), _num_invalid_input(0) {
...
#else
        const char *params[] = {"to_skip", "-i", "./src/workload/tpce/egen/flat/egen_flat_in/", "-l", "NULL", "-f",
                                sfe_str, "-w", wd_str, "-c", cust_str, "-t", cust_str};
    ...
    w_rc_t ShoreTPCEEnv::load_schema() {
        // create the schema
        _paccount_permission_desc = new account_permission_t();
        _pcustomer_desc = new customer_t();
        _pcustomer_account_desc = new customer_account_t();
        _pcustomer_taxrate_desc = new customer_taxrate_t();
        _pholding_desc = new holding_t();
        _pholding_history_desc = new holding_history_t();
        _pholding_summary_desc = new holding_summary_t();
        _pwatch_item_desc = new watch_item_t();
        _pwatch_list_desc = new watch_list_t();
        _pbroker_desc = new broker_t();
        _pcash_transaction_desc = new cash_transaction_t();
        _pcharge_desc = new charge_t();
        _pcommission_rate_desc = new commission_rate_t();
        _psettlement_desc = new settlement_t();
        _ptrade_desc = new trade_t();
        _ptrade_history_desc = new trade_history_t();
        _ptrade_request_desc = new trade_request_t();
        _ptrade_type_desc = new trade_type_t();
        _pcompany_desc = new company_t();
        _pcompany_competitor_desc = new company_competitor_t();
        _pdaily_market_desc = new daily_market_t();
        _pexchange_desc = new exchange_t();
        _pfinancial_desc = new financial_t();
        _pindustry_desc = new industry_t();
        _plast_trade_desc = new last_trade_t();
        _pnews_item_desc = new news_item_t();
        _pnews_xref_desc = new news_xref_t();
        _psector_desc = new sector_t();
        _psecurity_desc = new security_t();
        _paddress_desc = new address_t();
        _pstatus_type_desc = new status_type_t();
        _ptaxrate_desc = new taxrate_t();
        _pzip_code_desc = new zip_code_t();
