...
/* ------------------------------------------------------------- */
    ...
    account_permission_t::account_permission_t()
            : table_desc_t("ACCOUNT_PERMISSION", TPCE_ACCOUNT_PERMISSION_FCOUNT) //81B
    {
    ...
    customer_t::customer_t()
            : table_desc_t("CUSTOMER", TPCE_CUSTOMER_FCOUNT) //280B
    {
        ...
        create_primary_idx_desc(keys, 1); //unique
        create_index_desc("C_INDEX_2", 0, keys2, 2); //unique
        create_index_desc("C_INDEX_3", 0, keys3, 2); //unique
    }


    customer_account_t::customer_account_t()
            : table_desc_t("CUSTOMER_ACCOUNT", TPCE_CUSTOMER_ACCOUNT_FCOUNT) //85B
    {
        ...
        create_primary_idx_desc(keys, 1); //unique
        create_index_desc("CA_INDEX_2", keys2, 1, false, false); //non-unique
        //create_index_desc("CA_INDEX_3", keys3, 3, true, false);//unique
        //create_index_desc("CA_INDEX_4", keys4, 5, true, false);//unique
    }


    customer_taxrate_t::customer_taxrate_t()
            : table_desc_t("CUSTOMER_TAXRATE", TPCE_CUSTOMER_TAXRATE_FCOUNT) //13B
    {
        ...
        create_primary_idx_desc(keys, 2);; //unique
    }


    holding_t::holding_t()
            : table_desc_t("HOLDING", TPCE_HOLDING_FCOUNT) //52B
    {
        ...
        //create_primary_idx_desc(keys, 1); //unique
        //create_index_desc("H_INDEX_1", keys2, 4, true, false); //unique
        create_index_desc("H_INDEX_2", keys2, 3, false, false); //non-unique
    }


    holding_history_t::holding_history_t()
            : table_desc_t("HOLDING_HISTORY", TPCE_HOLDING_HISTORY_FCOUNT) //24B
    {
        ...
        //create_primary_idx_desc(keys, 2); //unique
        //create_index_desc("HH_INDEX_1", keys1, 2, true, false); //unique
        create_index_desc("HH_INDEX_2", keys2, 1, false, false); //non-unique
    }


    holding_summary_t::holding_summary_t()
            : table_desc_t("HOLDING_SUMMARY", TPCE_HOLDING_SUMMARY_FCOUNT) //28B
    {
    ...
    watch_item_t::watch_item_t()
            : table_desc_t("WATCH_ITEM", TPCE_WATCH_ITEM_FCOUNT) //24B
    {
    ...
    watch_list_t::watch_list_t()
            : table_desc_t("WATCH_LIST", TPCE_WATCH_LIST_FCOUNT) //16B
    {
        ...
        //create_primary_idx_desc(keys, 1); //unique
        create_primary_idx_desc(keys2, 2);; //unique
...
/* ------------------------------------------------------------- */
    ...
    broker_t::broker_t()
            : table_desc_t("BROKER", TPCE_BROKER_FCOUNT) //79B
    {
        ...
        create_primary_idx_desc(keys, 1);; //unique
        create_index_desc("B_INDEX_2", keys2, 2, true, false); //unique
        create_index_desc("B_INDEX_3", keys3, 2, true, false); //unique
    }


    cash_transaction_t::cash_transaction_t()
            : table_desc_t("CASH_TRANSACTION", TPCE_CASH_TRANSACTION_FCOUNT) //125B
    {
    ...
    charge_t::charge_t()
            : table_desc_t("CHARGE", TPCE_CHARGE_FCOUNT) //15B
    {
    ...
    commission_rate_t::commission_rate_t()
            : table_desc_t("COMMISSION_RATE", TPCE_COMMISSION_RATE_FCOUNT) //34B
    {
        ...
        create_primary_idx_desc(keys, 4); //unique
    }


    settlement_t::settlement_t()
            : table_desc_t("SETTLEMENT", TPCE_SETTLEMENT_FCOUNT) //65B
    {
    ...
    trade_t::trade_t()
            : table_desc_t("TRADE", TPCE_TRADE_FCOUNT) //145B
    {
        ...
        create_primary_idx_desc(keys, 1); //unique
        create_index_desc("T_INDEX_2", keys2, 3, true, false); //non-unique
        create_index_desc("T_INDEX_3", keys3, 2, false, false); //non-unique
    }


    trade_history_t::trade_history_t()
            : table_desc_t("TRADE_HISTORY", TPCE_TRADE_HISTORY_FCOUNT) //21B
    {
        ...
        //create_primary_idx_desc(keys, 2); //unique
        //create_index_desc("TH_INDEX_2", keys2, 1, false, false); //non-unique
        create_index_desc("TH_INDEX_3", keys3, 2, false, false); //non-unique
    }


    trade_request_t::trade_request_t()
            : table_desc_t("TRADE_REQUEST", TPCE_TRADE_REQUEST_FCOUNT) //49B
    {
        ...
        create_index_desc("TR_INDEX_4", keys4, 2, false, false); //non-unique
    }


    trade_type_t::trade_type_t()
            : table_desc_t("TRADE_TYPE", TPCE_TRADE_TYPE_FCOUNT) //20B
    {
...
/* ------------------------------------------------- */
    ...
    company_t::company_t()
            : table_desc_t("COMPANY", TPCE_COMPANY_FCOUNT) //298B
    {
        ...
        create_primary_idx_desc(keys1, 1); //unique
        create_index_desc("CO_INDEX_2", keys2, 2, true, false); //unique
        create_index_desc("CO_INDEX_3", keys3, 2, true, false); //unique
    }

    company_competitor_t::company_competitor_t()
            : table_desc_t("COMPANY_COMPETITOR", TPCE_COMPANY_COMPETITOR_FCOUNT) //21B
    {
        ...
        create_primary_idx_desc(keys1, 3); //unique
        //create_index_desc("CP_INDEX_2", keys2, 1, false, false); //non-unique
    }


    daily_market_t::daily_market_t()
            : table_desc_t("DAILY_MARKET", TPCE_DAILY_MARKET_FCOUNT) //52B
    {
        ...
        create_primary_idx_desc(keys1, 2); //unique
        create_index_desc("DM_INDEX_2", keys2, 2, true, false); //unique
    }


    exchange_t::exchange_t()
            : table_desc_t("EXCHANGE", TPCE_EXCHANGE_FCOUNT) //279B
    {
    ...
    financial_t::financial_t()
            : table_desc_t("FINANCIAL", TPCE_FINANCIAL_FCOUNT) //102B
    {
    ...
    industry_t::industry_t()
            : table_desc_t("INDUSTRY", TPCE_INDUSTRY_FCOUNT) //57B
    {
        ...
        create_primary_idx_desc(keys1, 1); //unique
        create_index_desc("IN_INDEX_2", keys2, 2, true, false); //unique
        create_index_desc("IN_INDEX_3", keys3, 2, true, false); //unique
    }


    last_trade_t::last_trade_t()
            : table_desc_t("LAST_TRADE", TPCE_LAST_TRADE_FCOUNT) //44B
    {
    ...
    news_item_t::news_item_t()
            : table_desc_t("NEWS_ITEM", TPCE_NEWS_ITEM_FCOUNT) //biggest Byte
    {
...
#warning should be 100000, but it fails in     _pnews_item_man   = new news_item_man_impl(_pnews_item_desc.get());
    ...
    news_xref_t::news_xref_t()
            : table_desc_t("NEWS_XREF", TPCE_NEWS_XREF_FCOUNT) //16B
    {
    ...
    sector_t::sector_t()
            : table_desc_t("SECTOR", TPCE_SECTOR_FCOUNT) //38B
    {
        ...
        // create_primary_idx_desc(keys, 1); //unique
        create_index_desc("SC_INDEX_2", keys2, 2, true, false); //unique
    }


    security_t::security_t()
            : table_desc_t("SECURITY", TPCE_SECURITY_FCOUNT) //197B
    {
        ...
        create_index_desc("S_INDEX_4", keys4, 3, true, false); //unique
    }
...
/* ------------------------------------------------------------- */
    ...
    address_t::address_t()
            : table_desc_t("ADDRESS", TPCE_ADDRESS_FCOUNT) //264B
    {
    ...
    status_type_t::status_type_t()
            : table_desc_t("STATUS_TYPE", TPCE_STATUS_TYPE_FCOUNT) //16B
    {
    ...
    taxrate_t::taxrate_t()
            : table_desc_t("TAXRATE", TPCE_TAXRATE_FCOUNT) //64B
    {
    ...
    zip_code_t::zip_code_t()
            : table_desc_t("ZIP_CODE", TPCE_ZIP_CODE_FCOUNT) //179B
    {
