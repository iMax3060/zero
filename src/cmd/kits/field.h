...
struct field_value_t {
    ...
    unsigned _real_size; /* current size of the value */
    unsigned _max_size;  /* maximum possible size of the buffer (shortcut) */
    ...
    inline bool is_setup() {
        return (_pfield_desc ? true : false);
    }

    /* access field description */
    inline field_desc_t* field_desc() {
        return (_pfield_desc);
    }
...
inline const char* field_desc_t::_set_keydesc() {
    ...
    switch (_type) {
        ...
        case SQL_CHAR:
        case SQL_INT:
            sprintf(_keydesc, "i%d", _size);
            break;

        case SQL_FLOAT:
        case SQL_LONG:
            sprintf(_keydesc, "f%d", _size);
            break;

        case SQL_VARCHAR:
            sprintf(_keydesc, "b*%d", _size);
            break;
...
inline void field_value_t::alloc_space(const unsigned len) {
    assert (_pfield_desc);
