...
public:
    ...
    desc_sort_buffer_t(const size_t field_count)
            : table_desc_t("DESC_SORT_BUF", field_count) {
    }
...
class desc_sort_man_impl : public table_man_t<desc_sort_buffer_t> {
...
public:
    ...
    desc_sort_man_impl(desc_sort_buffer_t *aSortBufferAsc, rep_row_t *aprow)
            : table_man_t<desc_sort_buffer_t>(aSortBufferAsc, false),
              _sort_buf(nullptr), _tuple_size(0), _tuple_count(0), _buf_size(0),
              _is_sorted(false), _preprow(aprow) {
    }
    ...
    /* return a sort iterator */
    w_rc_t get_desc_sort_iter(desc_sort_iter_impl *&desc_sort_iter);

    w_rc_t get_sort_iter(desc_sort_iter_impl *&sort_iter);
...
class desc_sort_iter_impl {
private:
    desc_sort_buffer_t *psortbuf;
    desc_sort_man_impl *_manager;
    int _index;

public:

    desc_sort_iter_impl(desc_sort_buffer_t *psortbuf, desc_sort_man_impl *psortman)
            : psortbuf(psortbuf), _manager(psortman), _index(0) {
        assert (_manager);
        assert (psortbuf);
        W_COERCE(open_scan());
...
inline w_rc_t desc_sort_iter_impl::next(bool &eof, table_row_t &tuple) {
    _manager->get_sorted(_index, &tuple);
    eof = (++_index > _manager->_tuple_count);
    return (RCOK);
}
