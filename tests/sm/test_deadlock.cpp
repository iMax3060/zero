...
//g_deadlock_dreadlock_interval_ms = 0;
    ...
    W_DO(test_env->btree_insert(stid, "a6", "data"));
...
TEST (DeadlockTest, ReadWriteLivelock) {
...
TEST (DeadlockTest, WriteReadLivelock) {
    test_env->empty_logdata_dir();
...
TEST (DeadlockTest, WriteReadWriteLivelock) {
    test_env->empty_logdata_dir();
...
TEST (DeadlockTest, WriteReadWriteDeadlock) {
    test_env->empty_logdata_dir();
...
TEST (DeadlockTest, ConversionDeadlock) {
    test_env->empty_logdata_dir();
...
w_rc_t indirect_conversion_deadlock(ss_m* ssm, test_volume_t *test_volume) {
    ...
    // write a5, (pause), read a3
    multiaccess_thread_t t4 (stid, "a5", true, "a2", false, "a3", false);
    W_DO(t4.fork());
    ::usleep (LONGTIME_USEC);
    EXPECT_TRUE(t3._done_multi[0]);
    EXPECT_FALSE(t3._done_multi[1]);
...
TEST (DeadlockTest, IndirectConversionDeadlock) {
    test_env->empty_logdata_dir();
...
w_rc_t complex1_deadlock(ss_m* ssm, test_volume_t *test_volume) {
    ...
    multiaccess_thread_t t4 (stid, "a3", false, "a2", false);
    multiaccess_thread_t t5 (stid, "a3", false, "a2", false);
    multiaccess_thread_t t6 (stid, "a3", false, "a2", false);
    multiaccess_thread_t t7 (stid, "a4", false, "a2", false);
    multiaccess_thread_t t8 (stid, "a4", false, "a2", false);
    W_DO(t4.fork());
    W_DO(t5.fork());
    W_DO(t6.fork());
    W_DO(t7.fork());
...
TEST (DeadlockTest, Complex1Deadlock) {
    test_env->empty_logdata_dir();
...
TEST (DeadlockTest, Complex2Deadlock) {
    test_env->empty_logdata_dir();
